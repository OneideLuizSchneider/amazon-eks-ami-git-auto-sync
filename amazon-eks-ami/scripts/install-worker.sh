...
sudo PCRE2_SYSTEM=y $TEMPLATE_DIR/ossec-hids-3.7.0/install.sh
sudo sed -i '/<global>/a \\    \<jsonout_output>yes<\/jsonout_output>' /var/ossec/etc/ossec.conf
sudo mv $TEMPLATE_DIR/ossec.service /etc/systemd/system/ossec.service
sudo systemctl daemon-reload
...
sudo /opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-ctl \
  -a fetch-config -m ec2 -s -c file:$TEMPLATE_DIR/ossec-cloudwatch-config.json
